...
const CardItem: React.FC<Props> = ({card, isShowed, onCardClick}) => {
  ...
  return (
    <>
      <Card sx={{width: '225px', borderRadius: '14px', boxShadow: isClicked ? '1px 0px 15px 3px #6A6A6A' : undefined}}
            onClick={handleClick}>
        <CardMedia
          component="img"
      ...
      {winningCard === card.code && isShowed &&
          <Typography textAlign="center" fontWeight="bold" mt={2}>This card is bigger!</Typography>}
    </>
  );
};
